...
static void printf_low_priority (Tree* tree_node, FILE* teh_output)
{
    ...
    if (tree_node->pre_ptr != NULL)
    {
        if ((tree_node->pre_ptr->value.type == OPERATION) && ((tree_node->pre_ptr->value.data == DIVISION) || (low_priority (tree_node->pre_ptr->value.data))))
        {
            exit = 1;
        }
        if ((tree_node->pre_ptr->value.type == OPERATION) && (tree_node->pre_ptr->value.data == POWER))
        {
            if (tree_node == tree_node->pre_ptr->left_ptr)
            {
                fprintf (teh_output, "(");
            }
            fprintf (teh_output, "{");
